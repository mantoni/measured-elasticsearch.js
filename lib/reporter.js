...
function sendBulk(client, collections, config, callback) {
  var now       = new Date(config.getTime());
  var indexDate = dateFormat(now, config.indexDateFormat, true);
  var index     = config.index + '-' + indexDate;
  ...
  client.bulk({ index : index, body : body }, callback);
}
...
exports.forClient = function (client, config) {
  ...
  if (config.pingTimeout) {
    pingParams.requestTimeout = config.pingTimeout;
  }
  reporter.start = function start(interval, unit) {
    client.ping(pingParams, function (err) {
      if (err) {
        reporter.emit('error', err);
